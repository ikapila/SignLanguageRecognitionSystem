...
class SelectorBIC(ModelSelector):
    ...
    def select(self):
        ...
        best_score, best_n_components, best_i_score = float('inf'), self.n_constant, float('inf')
        split_method = KFold()
...
class SelectorDIC(ModelSelector):
    ...
    def select(self):
        ...
        best_score, best_n_components = float('-inf'), self.n_constant
        ...
        for n in range(self.min_n_components, self.max_n_components+1):
            try:
                ...
                for word in other_words:
                    X_other, len_other = self.hwords[word]
                    other_logl += model.score(X_other, len_other)
                dv = logl - (1/(M-1)) * other_logl
                if dv > best_score:
                    best_score = dv
                    best_n_components = n
                # logsl.append(logl)
